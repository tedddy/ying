...
DROP TABLE `ying`.`hs_s_rt_EM`;
CREATE TABLE `hs_s_rt_EM` (
...
) ENGINE=InnoDB AUTO_INCREMENT=2787054 DEFAULT CHARSET=utf8;
...
DROP PROCEDURE `ying`.`hs_s_rt_EM`;
DELIMITER $$
...
DROP PROCEDURE `ying`.`hs_s_rt_EM_table`;
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `hs_s_rt_EM_table`(IN code VARCHAR(18), close VARCHAR(18), amount VARCHAR(18), volume VARCHAR(18), chgrate VARCHAR(18), WeiBi VARCHAR(18), chgrate5 VARCHAR(18), LiangBi VARCHAR(18), date VARCHAR(18), time VARCHAR(18), TimeSlot datetime)
BEGIN
SET code = IF(code = '', NULL, code);
SET close = IF(close = '', NULL, close);
SET amount = IF(amount = '', NULL, amount);
...
if (code in (select code from hs_s where f300 = 1)) then INSERT INTO `hs_s_hs300_rt_EM` (`code`, `close`, `amount`, `volume`, `chgrate`, `WeiBi`, `chgrate5`, `LiangBi`, `date`, `time`, `TimeSlot`) VALUES (code, close, amount, volume, chgrate, WeiBi, chgrate5, LiangBi, date, time, TimeSlot) 
ON DUPLICATE KEY UPDate 
`code` = code,
`close` = close,
`amount` = amount,
...
if (code in (select code from hs_s where f50SH = 1)) then INSERT INTO `hs_s_sh50_rt_EM` (`code`, `close`, `amount`, `volume`, `chgrate`, `WeiBi`, `chgrate5`, `LiangBi`, `date`, `time`, `TimeSlot`) VALUES (code, close, amount, volume, chgrate, WeiBi, chgrate5, LiangBi, date, time, TimeSlot) 
ON DUPLICATE KEY UPDate 
`code` = code,
`close` = close,
`amount` = amount,
...
if (code in (select code from hs_s where f500 = 1)) then INSERT INTO `hs_s_zz500_rt_EM` (`code`, `close`, `amount`, `volume`, `chgrate`, `WeiBi`, `chgrate5`, `LiangBi`, `date`, `time`, `TimeSlot`) VALUES (code, close, amount, volume, chgrate, WeiBi, chgrate5, LiangBi, date, time, TimeSlot) 
ON DUPLICATE KEY UPDate 
`code` = code,
`close` = close,
`amount` = amount,
