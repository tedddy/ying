DROP TABLE `hs_s_rt_EM_test`;
CREATE TABLE `hs_s_rt_EM_test` (
  `id` MEDIUMINT(8) UNSIGNED NOT NULL AUTO_INCREMENT,
  `idsub` MEDIUMINT(8) UNSIGNED DEFAULT NULL,
...
DECLARE SMA_SUM DECIMAL(8,2); -- variable for total of price of peorids
DECLARE close_tmp DECIMAL(6,2); -- variable for cursor fetch into
DECLARE cur_close CURSOR FOR
	 SELECT close FROM `hs_s_rt_EM_test` WHERE `code` = code ORDER BY TimeSlot desc;	
...
DROP PROCEDURE `hs_s_rt_EM_test`;
-- The procdure is Not working for idsub too! tedd why?
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `hs_s_rt_EM_test`(IN code VARCHAR(18), close VARCHAR(18), amount VARCHAR(18), volume VARCHAR(18), chgrate VARCHAR(18), WeiBi VARCHAR(18), chgrate5 VARCHAR(18), LiangBi VARCHAR(18), date VARCHAR(18), time VARCHAR(18), TimeSlot DATETIME, idsub INT(10))
BEGIN

DECLARE loop_cnt INT; -- variable for loop counter
DECLARE record_not_found INTEGER DEFAULT 0;
DECLARE SMA_SUM INT; -- variable for total of price of days
DECLARE idsub_tmp INT; -- variable for cursor fetch into
DECLARE cur_idsub CURSOR FOR
	 SELECT idsub FROM `hs_s_rt_EM_test` WHERE `code` = code ORDER BY TimeSlot desc;	
DECLARE CONTINUE HANDLER FOR NOT FOUND SET record_not_found = 1;-- initialize variables for use in procedure
SET loop_cnt = 0;
SET  SMA_SUM = 0;
open cur_idsub;
	the_loop: LOOP
		...
		IF record_not_found THEN
		  LEAVE the_loop;
		END IF;
		SET SMA_SUM = SMA_SUM + idsub_tmp; -- add the sum to SMA_SUM
	END LOOP the_loop;
	SET SMA_SUM = ROUND(((SMA_SUM + idsub_tmp) / 5),2);
close cur_idsub;
SET idsub = SMA_SUM;
SET code = IF(code = '', NULL, code);
SET close = IF(close = '', NULL, close);
...
INSERT INTO `hs_s_rt_EM_test` (`code`, `close`, `amount`, `volume`, `chgrate`, `WeiBi`, `chgrate5`, `LiangBi`, `date`, `time`, `TimeSlot`, `idsub`) VALUES (code, close, amount, volume, chgrate, WeiBi, chgrate5, LiangBi, date, time, TimeSlot, idsub) 
ON DUPLICATE KEY UPDATE 
`code` = code,
`close` = close,
`amount` = amount,
...
DROP PROCEDURE `hs_s_rt_EM_test`;
-- The procdure is Not working for idsub! tedd why?
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `hs_s_rt_EM_test`(IN code VARCHAR(18), close VARCHAR(18), amount VARCHAR(18), volume VARCHAR(18), chgrate VARCHAR(18), WeiBi VARCHAR(18), chgrate5 VARCHAR(18), LiangBi VARCHAR(18), date VARCHAR(18), time VARCHAR(18), TimeSlot DATETIME, idsub MEDIUMINT(8))
BEGIN

DECLARE idsub_tmp MEDIUMINT; -- variable for cursor fetch into
DECLARE idsub_out MEDIUMINT; -- variable outside of the cursor 
DECLARE cur_idsub CURSOR FOR
	 SELECT round(avg(idsub)) FROM `hs_s_rt_EM_test` WHERE `code` = code;
set idsub_out = 0; -- initialize variable outside of the cursor 	
open cur_idsub;
FETCH cur_idsub into idsub_tmp;
set idsub_out = idsub_tmp;
close cur_idsub;
SET idsub = 1 + idsub_out;
SET code = IF(code = '', NULL, code);
SET close = IF(close = '', NULL, close);
SET amount = IF(amount = '', NULL, amount);
SET volume = IF(volume = '', NULL, volume);
