...
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `hs_s_xts_fast`(IN code VARCHAR(6), date DATE, open DECIMAL(7,3), high DECIMAL(7,3), low DECIMAL(7,3), close DECIMAL(7,3), volume BIGINT(11) UNSIGNED, amount BIGINT(12) UNSIGNED, time VARCHAR(9))
BEGIN
...
INSERT INTO `hs_s_xts_fast` (`code`, `date`, `open`, `high`, `low`, `close`, `volume`, `amount`, `time`) VALUES (code, date, open, high, low, close, volume, amount, time) 
...
CALL hs_s_xts_fast('[标签:code]', '[系统时间转化:yyyy-MM-dd]', '[标签:open]', '[标签:high]', '[标签:low]', '[标签:close]', '[标签:volume]', '[标签:amount]', '[标签:time]') 
