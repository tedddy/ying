...
DROP TABLE `hk_w_rt_lag`;
CREATE TABLE `hk_w_rt_lag` (
...
-- DROP PROCEDURE hk_w_rt_lag;
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `hk_w_rt_lag`(IN code CHAR(5), close DECIMAL(7,3), volume  VARCHAR(18), turnover VARCHAR(18), TimeSlot VARCHAR(18))
BEGIN
...
INSERT INTO `hk_w_rt_lag` (`code`, `close`, `volume`, `turnover`, `TimeSlot`) VALUES (code, close, volume, turnover, TimeSlot) 
...
CALL hk_w_rt_lag('[标签:code]', '[标签:close]', '[标签:volume]', '[标签:turnover]', '');
