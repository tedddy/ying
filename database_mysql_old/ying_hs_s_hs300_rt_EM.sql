SELECT * FROM ying.hs_s_hs300_rt_EM WHERE
	
            (`date` = curdate())
            AND (`chgrate` > 0)
            AND (`close` >= ROUND((1.1 * ((100 * `close`) / (100 + `chgrate`))),
                2));
-- DELETE FROM `ying`.`hs_s_hs300_rt_EM` WHERE `id`<='*' and `id`>='*';

SELECT * FROM ying.hs_s_hs300_rt_EM where code = '600000';

SELECT * FROM ying.hs_s_hs300_rt_EM where date = curdate() order by id desc;

SELECT * FROM ying.hs_s_hs300_rt_EM where TimeSlot = '2015-09-08 15:05:00' order by id desc ;

SELECT * FROM ying.hs_s_hs300_rt_EM WHERE date = curdate() and time <= '09:35:00' and time > '09:20:00' order by id desc;
-- UPDATE `ying`.`hs_s_hs300_rt_EM` SET `TimeSlot`='11:25:00' WHERE date = curdate() and time < '11:25:00'and time > '11:20:00';

UPDATE `ying`.`hs_s_hs300_rt_EM` SET `datetime`=cast(CONCAT(date,' ',TimeSlot) as datetime);

SELECT * FROM ying.hs_s_hs300_rt_EM where date = curdate() and time < '10:48:00'and time > '10:35:00';

UPDATE `ying`.`hs_s_hs300_rt_EM` SET `TimeSlot`= '09:50:00' where date = curdate() and time < '09:50:00'and time > '09:45:00';

SELECT * FROM ying.hs_s_hs300_rt_EM order by id desc;
...
UPDATE `ying`.`hs_s_hs300_rt_EM` SET TimeSlot =  CASE WHEN time <= '09:27:00' THEN '09:25:00' WHEN time <= '09:32:00' THEN '09:30:00' WHEN time <= '09:37:00' THEN '09:35:00' WHEN time <= '09:42:00' THEN '09:40:00' WHEN time <= '09:47:00' THEN '09:45:00' WHEN time <= '09:52:00' THEN '09:50:00' WHEN time <= '09:57:00' THEN '09:55:00' WHEN time <= '10:02:00' THEN '10:00:00' WHEN time <= '10:07:00' THEN '10:05:00' WHEN time <= '10:12:00' THEN '10:10:00' WHEN time <= '10:17:00' THEN '10:15:00' WHEN time <= '10:22:00' THEN '10:20:00' WHEN time <= '10:27:00' THEN '10:25:00' WHEN time <= '10:32:00' THEN '10:30:00' WHEN time <= '10:37:00' THEN '10:35:00' WHEN time <= '10:42:00' THEN '10:40:00' WHEN time <= '10:47:00' THEN '10:45:00' WHEN time <= '10:52:00' THEN '10:50:00' WHEN time <='10:57:00' THEN '10:55:00' WHEN time <='11:02:00' THEN '11:00:00' WHEN time <='11:07:00' THEN '11:05:00' WHEN time <='11:12:00' THEN '11:10:00' WHEN time <='11:17:00' THEN '11:15:00' WHEN time <='11:22:00' THEN '11:20:00' WHEN time <='11:27:00' THEN '11:25:00' WHEN time <='11:32:00' THEN '11:30:00' WHEN (time <='13:07:00' and time > '13:00:00') THEN '13:05:00' WHEN time <='13:12:00' THEN '13:10:00' WHEN time <='13:17:00' THEN '13:15:00' WHEN time <='13:22:00' THEN '13:20:00' WHEN time <='13:27:00' THEN '13:25:00' WHEN time <='13:32:00' THEN '13:30:00' WHEN time <='13:37:00' THEN '13:35:00' WHEN time <='13:42:00' THEN '13:40:00' WHEN time <='13:47:00' THEN '13:45:00' WHEN time <='13:52:00' THEN '13:50:00' WHEN time <='13:57:00' THEN '13:55:00' WHEN time <='14:02:00' THEN '14:00:00' WHEN time <='14:07:00' THEN '14:05:00' WHEN time <='14:12:00' THEN '14:10:00' WHEN time <='14:17:00' THEN '14:15:00' WHEN time <='14:22:00' THEN '14:20:00' WHEN time <='14:27:00' THEN '14:25:00' WHEN time <='14:32:00' THEN '14:30:00' WHEN time <='14:37:00' THEN '14:35:00' WHEN time <='14:42:00' THEN '14:40:00' WHEN time <='14:47:00' THEN '14:45:00' WHEN time <='14:52:00' THEN '14:50:00' WHEN time <='14:57:00' THEN '14:55:00' WHEN time <='15:01:00' THEN '15:00:00' WHEN time <='15:07:00' THEN '15:05:00' end where date = curdate();

-- UPDATE `ying`.`hs_s_hs300_rt_EM` SET TimeSlot = NULL;

UPDATE `ying`.`hs_s_hs300_rt_EM` SET TimeSlot = time(datetime); -- tedd
...
FROM
    ...
    GROUP BY code) AS latest
        JOIN
    ying.hs_s_hs300_rt_EM AS rt300 ON latest.code = rt300.code
...
-- 沪深300股票的加权5分钟涨速指数
SELECT * FROM `ying`.`hs_s_hs300_chgrate5_weight`;
...
VIEW `hs_s_hs300_chgrate5_weight` AS
    ...
    FROM
        ...
        JOIN `hs_index_gp_info_EM` `c` ON (((`a`.`code` = `c`.`code`)
            AND (`c`.`code_index` = '000300'))))
    GROUP BY `a`.`TimeSlot`
    ORDER BY `a`.`TimeSlot` DESC , SUM((`a`.`chgrate5` * `c`.`weight`)) DESC;

    
    select sum(weight) from `ying`.`hs_index_gp_info_EM` where `code_index` = '000300';
    ...
    FROM
        (`hs_s_hs300_rt_EM` `a`
        JOIN (select * from `ying`.`hs_index_gp_info_EM` where `code_index` = '399300') `c` ON ((`a`.`code` = `c`.`code`)))
    GROUP BY `a`.`TimeSlot`
    ORDER BY `a`.`TimeSlot` DESC , SUM((`a`.`chgrate5` * `c`.`weight`)) DESC;
...
VIEW `hs_s_hs300_d_weight` AS
    ...
    FROM
        (`hs_s_hs300_rt_EM` `a`JOIN `hs_index_gp_info_EM` `c` ON (((`a`.`code` = `c`.`code`)
            AND (`c`.`code_index` = '000300'))))
    WHERE
        (`a`.`chgrate` < 0)
    GROUP BY `a`.`TimeSlot`
...
VIEW `hs_s_hs300_z_d_No` AS
    ...
    FROM
        `hs_s_hs300_rt_EM`
    GROUP BY `hs_s_hs300_rt_EM`.`TimeSlot`
    ORDER BY `hs_s_hs300_rt_EM`.`TimeSlot` DESC;
...
VIEW `hs_s_sh50_rt_EM_view` AS
    ...
    FROM
        (`hs_s` `b`
        LEFT JOIN `hs_s_hs300_rt_EM` `a` ON (((`a`.`code` = `b`.`code`)
            AND (`b`.`f50SH` = 1))))
    WHERE
        (`a`.`id` IS NOT NULL)
    ORDER BY `a`.`TimeSlot` DESC;
...
-- VIEW `hs_s_sh50_rt_EM_view_1` AS
    SELECT 
        `hs_s_hs300_rt_EM`.`id` AS `id`,
        `hs_s_hs300_rt_EM`.`code` AS `code`,
        `hs_s_hs300_rt_EM`.`close` AS `close`,
        `hs_s_hs300_rt_EM`.`amount` AS `amount`,
        `hs_s_hs300_rt_EM`.`volume` AS `volume`,
        `hs_s_hs300_rt_EM`.`chgrate` AS `chgrate`,
        `hs_s_hs300_rt_EM`.`WeiBi` AS `WeiBi`,
        `hs_s_hs300_rt_EM`.`chgrate5` AS `chgrate5`,
        `hs_s_hs300_rt_EM`.`LiangBi` AS `LiangBi`,
        `hs_s_hs300_rt_EM`.`date` AS `date`,
        `hs_s_hs300_rt_EM`.`time` AS `time`,
        `hs_s_hs300_rt_EM`.`TimeSlot` AS `TimeSlot`
    FROM
        `hs_s_hs300_rt_EM`
    WHERE
        `hs_s_hs300_rt_EM`.`code` IN (SELECT DISTINCT
                `a`.`code`
            FROM
                ((`hs_bk_gp_info_EM` `a`
