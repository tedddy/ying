...
CREATE TABLE `fund_b_rt` (
	...
	`l_p` decimal(6,3) unsigned DEFAULT NULL COMMENT '[参数9]: 价格杠杆 fundb_price_leverage_rt old_field_name: leverate_p',
	`l_j` decimal(6,3) unsigned DEFAULT NULL COMMENT '[参数10]: 净值杠杆 fundb_net_leverage_rt old_field_name: leverate_j',
...
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `fund_b_rt`(IN idf VARCHAR(18), close VARCHAR(18), chgrate VARCHAR(18), chgrate_i VARCHAR(18), amount VARCHAR(18), jjjz VARCHAR(18), jjjz_est VARCHAR(18), yjl VARCHAR(18), jjjz_m VARCHAR(18), yjl_m_est VARCHAR(18), l_p VARCHAR(18), l_j VARCHAR(18), d VARCHAR(18), t VARCHAR(18))
BEGIN
...
INSERT INTO `fund_b_rt` (`idf`, `close`, `chgrate`, `chgrate_i`, `amount`, `jjjz`, `jjjz_est`, `jjjz_m`, `yjl_m_est`, `l_p`, `l_j`, `dt`) VALUES (idf, close, chgrate, chgrate_i, amount, jjjz, jjjz_est, jjjz_m, yjl_m_est, l_p, l_j, dt) 
...
CALL fund_b_rt('[标签:idf]','[标签:close]','[标签:chgrate]','[标签:chgrate_i]','[标签:amount]','[标签:jjjz]','[标签:jjjz_est]','[标签:jjjz_m]','[标签:yjl_m_est]','[标签:l_p]','[标签:l_j]','[标签:d]','[标签:t]');

-- ying locoy project fund_b_rt
-- 
