...
-- DROP PROCEDURE `s_xts_adj_hst_sina`;
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `s_xts_adj_hst_sina`(IN ids VARCHAR(6), `dt` DATE, open DECIMAL(10,3) UNSIGNED, high DECIMAL(10,3) UNSIGNED, low DECIMAL(10,3) UNSIGNED, close DECIMAL(10,3) UNSIGNED, volume DECIMAL(15,3) UNSIGNED, amount DECIMAL(16,3) UNSIGNED, fa DECIMAL(10,3) UNSIGNED)
...
BEGIN
	...
	IF volume > 0 AND volume IS NOT NULL THEN
		INSERT INTO `s_xts_adj_hst_sina` (`ids`, `dt`, `open`, `high`, `low`, `close`, `volume`, `amount`, `fa`) VALUES (ids, `dt`, open, high, low, close, volume, amount, fa) 
...
CALL s_xts_adj_hst_sina('[标签:ids]','[标签:`dt`]','[标签:open]','[标签:high]','[标签:low]','[标签:close]','[标签:volume]','[标签:amount]','[标签:fa]');
