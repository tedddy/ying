...
DELIMITER $$
USE `ying`$$
CREATE DEFINER=`gxh`@`%` PROCEDURE `index_rt`(IN idi VARCHAR(6), close DECIMAL(9,2), amount BIGINT(12) UNSIGNED, volume BIGINT(11) UNSIGNED, time VARCHAR(9))
BEGIN
DECLARE `dt` DATETIME;
...
INSERT INTO `index_rt` (`idi`, `close`, `amount`, `volume`, `dt`) VALUES (idi, close, amount, volume, dt) 
...
CALL index_rt('[标签:idi]', '[标签:close]', '[标签:amount]', '[标签:volume]', '[标签:time]');
-- history
-- CREATE TABLE `index_rt` (
--   `idi` VARCHAR(6) NOT NULL COMMENT 'index id',
--   `close` DECIMAL(9,2) UNSIGNED DEFAULT NULL COMMENT '收盘价',
...
-- ) ENGINE=INNODB DEFAULT CHARSET=UTF8;
-- 
-- ALTER TABLE `ying`.`index_rt` 
-- CHANGE COLUMN `volume` `amount` INT(8) UNSIGNED NULL COMMENT '成交量' ,
-- CHANGE COLUMN `amount` `volume` INT(9) UNSIGNED NULL COMMENT '股票成交额' ;
