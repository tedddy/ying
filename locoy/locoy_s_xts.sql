...
BEGIN
	...
	IF volume > 0 AND volume IS NOT NULL THEN
		INSERT INTO `s_xts` (`ids`, `d`, `open`, `high`, `low`, `close`, `volume`, `amount`) VALUES (ids, d, open, high, low, close, volume, amount) 
		ON DUPLICATE KEY UPDATE
			...
			`volume` = volume,
			`amount` = amount;
		INSERT INTO `ying_calc`.`s_xts_adj` (`ids`, `d`, `open`, `high`, `low`, `close`, `volume`, `amount`) VALUES (ids, d, open, high, low, close, volume, amount) 
		ON DUPLICATE KEY UPDATE
			`open` = open,
			`high` =high,
			`low` = low,
