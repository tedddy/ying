...
-- DROP PROCEDURE `s_rt`;
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `s_rt`(IN code VARCHAR(18), close VARCHAR(18), amount VARCHAR(18), volume VARCHAR(18), chgrate VARCHAR(18), WeiBi VARCHAR(18), chgrate5 VARCHAR(18), LiangBi VARCHAR(18), date VARCHAR(18), time VARCHAR(18))
BEGIN
DECLARE dt DATETIME;
...
INSERT INTO `s_rt` (`ids`, `close`, `amount`, `volume`, `chgrate`, `WeiBi`, `chgrate5`, `LiangBi`, `dt`) VALUES (code, close, amount, volume, chgrate, WeiBi, chgrate5, LiangBi, dt) 
...
insert into `s_rt_sma` (`dt`,`ids`,`close5`,`close10`,`close20`,`close30`,`close60`,`close120`) values (dt,code,@out_sma_1, @out_sma_2, @out_sma_3, @out_sma_4, @out_sma_5, @out_sma_6) ON DUPLICATE KEY UPDATE `close5` =  @out_sma_1, `close10` =  @out_sma_2, `close20` =  @out_sma_3, `close30` =  @out_sma_4, `close60` =  @out_sma_5, `close120` =  @out_sma_6;
END$$
DELIMITER ;
