/* 	
	...
	execute file proc_s_xts_adj_ids_d_close_sma.sql & test all procedures
*/
-- file name: E:\user_tony\Documents\GitHub\ying\proc\sma_TMPL\proc_s_xts_adj_ids_d_close_sma.sql
...
-- Test the proc
-- 	CALL `s_xts_adj_ids_d_close_sma`('2019-09-09', '601318', 10, @out_sma);
-- 	SELECT @out_sma;    
...
DROP PROCEDURE IF EXISTS `s_xts_adj_ids_d_close_sma`;
...
-- 	DECLARE coursors 
		DECLARE cursor1 CURSOR FOR -- variable for the first cursor. 
			SELECT `close` FROM `s_xts_adj` WHERE `ids` = in_ids and `d` <= in_d ORDER BY `d` DESC limit limit_number_for_cursor;
...
DROP PROCEDURE IF EXISTS `s_xts_adj_ids_d_close_sma_multiPeriods`;
...
--  sma_1
	CALL `s_xts_adj_ids_d_close_sma`(in_d, in_ids, in_smaPeriods_1, @out_sma_1);		
	SET out_sma_1 = @out_sma_1;
    
--  sma_2
	CALL `s_xts_adj_ids_d_close_sma`(in_d, in_ids, in_smaPeriods_2, @out_sma_2);		
	SET out_sma_2 = @out_sma_2;

--  sma_3
	CALL `s_xts_adj_ids_d_close_sma`(in_d, in_ids, in_smaPeriods_3, @out_sma_3);		
	SET out_sma_3 = @out_sma_3;

--  sma_4
	CALL `s_xts_adj_ids_d_close_sma`(in_d, in_ids, in_smaPeriods_4, @out_sma_4);		
	SET out_sma_4 = @out_sma_4;

--  sma_5
	CALL `s_xts_adj_ids_d_close_sma`(in_d, in_ids, in_smaPeriods_5, @out_sma_5);		
	SET out_sma_5 = @out_sma_5;

--  sma_6
	CALL `s_xts_adj_ids_d_close_sma`(in_d, in_ids, in_smaPeriods_6, @out_sma_6);		
...
-- 	CALL `s_xts_adj_ids_d_close_sma_multiPeriods_loop_dt_ids`('2005-10-13 13:05:00', '2019-10-15 15:05:00', 5, 10, 20, 30, 60, 120);     
-- 	SELECT * FROM `s_xts_adj_sma` WHERE `d` >= '2005-10-13 13:05:00' AND `d` <= '2019-10-15 15:05:00' ORDER BY `d` DESC;
-- 	select `d` FROM `s_xts_adj`;
-- instructions for PROCEDURE `s_xts_adj_ids_d_close_sma_multiPeriods_loop_dt_ids`
...
DROP PROCEDURE IF EXISTS `s_xts_adj_ids_d_close_sma_multiPeriods_loop_dt_ids`; 
-- This proc loops through two fields to compute sma's
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `s_xts_adj_ids_d_close_sma_multiPeriods_loop_dt_ids`(
...
-- 	open cursor
	...
	-- 	Loop of cursor  
		...
		-- 	actions	
			...
			INSERT INTO `s_xts_adj_sma` (`d`,`ids`,`close5`,`close10`,`close20`,`close30`,`close60`,`close120`) VALUES (cursor_fetch_tmp_dt, cursor_fetch_tmp_ids, @out_sma_1, @out_sma_2, @out_sma_3, @out_sma_4, @out_sma_5, @out_sma_6) ON DUPLICATE KEY UPDATE `close5` =  @out_sma_1, `close10` =  @out_sma_2, `close20` =  @out_sma_3, `close30` =  @out_sma_4, `close60` =  @out_sma_5, `close120` =  @out_sma_6;
