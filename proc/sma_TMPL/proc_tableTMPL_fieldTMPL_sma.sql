/* 	
	...
	save file proc_tableTMPL_fieldTMPL_sma.sql as another file accordingly
	delete `ying`.
	replace
	...
	execute file proc_tableTMPL_fieldTMPL_sma.sql & test all procedures
*/
-- file name: E:\user_tony\Documents\GitHub\ying\proc\sma_TMPL\proc_tableTMPL_fieldTMPL_sma

-- instructions for PROCEDURE `tableTMPL_fieldTMPL_sma`
...
DROP PROCEDURE IF EXISTS `ying_calc`.`tableTMPL_fieldTMPL_sma`;
-- This procedure compute sma (simple moving average) for stock (idTMPL) at given datetime (`dtTMPL`).
DELIMITER $$ 
CREATE PROCEDURE `ying_calc`.`tableTMPL_fieldTMPL_sma`(
	IN 	
...
-- Test the proc
-- CALL `ying_calc`.`tableTMPL_fieldTMPL_sma`('2015-10-28 15:05:00', '000001', 10, @out_sma);
-- SELECT @out_sma;    


DROP PROCEDURE IF EXISTS `ying_calc`.`tableTMPL_fieldTMPL_sma_multiPeriods`;

DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `ying_calc`.`tableTMPL_fieldTMPL_sma_multiPeriods`
	(
	IN 	
		in_dtTMPL DATETIME, -- variable datetime. If the value of this variable is large (such as '2019-09-09'), then the sma we get from this proc is for the latest datetime in table tableTMPL.  
		in_idTMPL VARCHAR(6),
...
--  sma_1
	CALL `ying_calc`.`tableTMPL_fieldTMPL_sma`(in_dtTMPL, in_idTMPL, in_smaPeriods_1, @out_sma_1);		
	SET out_sma_1 = @out_sma_1;
    
--  sma_2
	CALL `ying_calc`.`tableTMPL_fieldTMPL_sma`(in_dtTMPL, in_idTMPL, in_smaPeriods_2, @out_sma_2);		
	SET out_sma_2 = @out_sma_2;

--  sma_3
	CALL `ying_calc`.`tableTMPL_fieldTMPL_sma`(in_dtTMPL, in_idTMPL, in_smaPeriods_3, @out_sma_3);		
	SET out_sma_3 = @out_sma_3;

--  sma_4
	CALL `ying_calc`.`tableTMPL_fieldTMPL_sma`(in_dtTMPL, in_idTMPL, in_smaPeriods_4, @out_sma_4);		
	SET out_sma_4 = @out_sma_4;

--  sma_5
	CALL `ying_calc`.`tableTMPL_fieldTMPL_sma`(in_dtTMPL, in_idTMPL, in_smaPeriods_5, @out_sma_5);		
	SET out_sma_5 = @out_sma_5;

--  sma_6
	CALL `ying_calc`.`tableTMPL_fieldTMPL_sma`(in_dtTMPL, in_idTMPL, in_smaPeriods_6, @out_sma_6);		
...
--     
	CALL `ying_calc`.`tableTMPL_fieldTMPL_sma_multiPeriods`('2019-10-28 00:00:00', '601318', 5, 10, 20, 30, 60, 120, @out_sma_1, @out_sma_2, @out_sma_3, @out_sma_4, @out_sma_5, @out_sma_6);
	SELECT @out_sma_1, @out_sma_2, @out_sma_3, @out_sma_4, @out_sma_5, @out_sma_6;

    
DROP PROCEDURE IF EXISTS `ying_calc`.`tableTMPL_fieldTMPL_sma_multiPeriods_loop_dt_ids`; 
-- This proc loops through two fields to compute sma's
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `ying_calc`.`tableTMPL_fieldTMPL_sma_multiPeriods_loop_dt_ids`(
...
-- 	open cursor
	...
	-- 	Loop of cursor  
		...
		-- 	actions	
			CALL `ying_calc`.`tableTMPL_fieldTMPL_sma_multiPeriods`(cursor_fetch_tmp_dt, cursor_fetch_tmp_ids,5,10,20,30,60,120, @out_sma_1,@out_sma_2,@out_sma_3,@out_sma_4,@out_sma_5,@out_sma_6);
...
--  test query in the proc    
	...
	CALL `ying_calc`.`tableTMPL_fieldTMPL_sma_multiPeriods_loop_dt_ids`('2015-08-12 15:05:00' , '2016-08-08 15:05:00', 5, 10, 20, 30, 60, 120);     

        CALL `ying_calc`.`tableTMPL_fieldTMPL_sma_multiPeriods_loop_dt_ids`(DATE_SUB(CURRENT_TIMESTAMP(), INTERVAL 3 DAY) , '2019-10-15 15:05:00', 5, 10, 20, 30, 60, 120);     
