/* 	
	replace `ying_calc`.`index_xts` with ``, `idi` with ``, `dt` with ``, `cjezb` with ``
	save file proc_index_xts_cjezb_sma.sql as another file accordingly
	delete `ying`.
	replace
	...
	execute file proc_index_xts_cjezb_sma.sql & test all procedures
*/
-- file name: E:\user_tony\Documents\GitHub\ying\proc\sma_TMPL\proc_index_xts_cjezb_sma

-- instructions for PROCEDURE `ying_calc`.`index_xts_cjezb_sma`
...
-- 
ALTER TABLE `ying_calc`.`index_xts_sma`
ADD COLUMN `cjezb5` DECIMAL(6,2) UNSIGNED DEFAULT NULL COMMENT 'cjezb 5 periods均线',
ADD COLUMN `cjezb10` DECIMAL(6,2) UNSIGNED DEFAULT NULL COMMENT 'cjezb 10 periods均线',
ADD COLUMN  `cjezb20` DECIMAL(6,2) UNSIGNED DEFAULT NULL COMMENT 'cjezb 20 periods均线',
ADD COLUMN  `cjezb30` DECIMAL(6,2) UNSIGNED DEFAULT NULL COMMENT 'cjezb 30 periods均线',
ADD COLUMN  `cjezb60` DECIMAL(6,2) UNSIGNED DEFAULT NULL COMMENT 'cjezb 60 periods均线',
ADD COLUMN  `cjezb120` DECIMAL(6,2) UNSIGNED DEFAULT NULL COMMENT 'cjezb 120 periods均线';

DROP PROCEDURE IF EXISTS `ying_calc`.`index_xts_cjezb_sma`;
-- This procedure compute sma (simple moving average) for stock (idi) at given datetime (`dt`).
DELIMITER $$ 
CREATE PROCEDURE `ying_calc`.`index_xts_cjezb_sma`(
	IN 	
...
-- 	DECLARE coursors 
		DECLARE cursor1 CURSOR FOR -- variable for the first cursor. 
			SELECT `cjezb` FROM `ying_calc`.`index_xts` WHERE `idi` = in_idi AND `dt` <= in_dt ORDER BY `dt` DESC LIMIT LIMIT_NUMBER_FOR_CURSOR;
...
-- Test the proc
	CALL `ying_calc`.`index_xts_cjezb_sma`('2015-10-28 15:05:00', '000001', 10, @out_sma);
	SELECT @out_sma;    


DROP PROCEDURE IF EXISTS `ying_calc`.`index_xts_cjezb_sma_multiPeriods`;

DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `ying_calc`.`index_xts_cjezb_sma_multiPeriods`
	(
	IN 	
		in_dt DATETIME, -- variable datetime. If the value of this variable is large (such as '2019-09-09'), then the sma we get from this proc is for the latest datetime in table index_xts.  
		in_idi VARCHAR(6),
...
--  sma_1
	CALL `ying_calc`.`index_xts_cjezb_sma`(in_dt, in_idi, in_smaPeriods_1, @out_sma_1);		
	SET out_sma_1 = @out_sma_1;
    
--  sma_2
	CALL `ying_calc`.`index_xts_cjezb_sma`(in_dt, in_idi, in_smaPeriods_2, @out_sma_2);		
	SET out_sma_2 = @out_sma_2;

--  sma_3
	CALL `ying_calc`.`index_xts_cjezb_sma`(in_dt, in_idi, in_smaPeriods_3, @out_sma_3);		
	SET out_sma_3 = @out_sma_3;

--  sma_4
	CALL `ying_calc`.`index_xts_cjezb_sma`(in_dt, in_idi, in_smaPeriods_4, @out_sma_4);		
	SET out_sma_4 = @out_sma_4;

--  sma_5
	CALL `ying_calc`.`index_xts_cjezb_sma`(in_dt, in_idi, in_smaPeriods_5, @out_sma_5);		
	SET out_sma_5 = @out_sma_5;

--  sma_6
	CALL `ying_calc`.`index_xts_cjezb_sma`(in_dt, in_idi, in_smaPeriods_6, @out_sma_6);		
...
--     
	CALL `ying_calc`.`index_xts_cjezb_sma_multiPeriods`('2019-10-28 00:00:00', '000001', 5, 10, 20, 30, 60, 120, @out_sma_1, @out_sma_2, @out_sma_3, @out_sma_4, @out_sma_5, @out_sma_6);
	SELECT @out_sma_1, @out_sma_2, @out_sma_3, @out_sma_4, @out_sma_5, @out_sma_6;

    
DROP PROCEDURE IF EXISTS `ying_calc`.`index_xts_cjezb_sma_multiPeriods_loop_dt_idi`; 
-- This proc loops through two fields to compute sma's
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `ying_calc`.`index_xts_cjezb_sma_multiPeriods_loop_dt_idi`(
...
-- 	open cursor
	...
	-- 	Loop of cursor  
		...
		-- 	actions	
			CALL `ying_calc`.`index_xts_cjezb_sma_multiPeriods`(cursor_fetch_tmp_dt, cursor_fetch_tmp_idi,5,10,20,30,60,120, @out_sma_1,@out_sma_2,@out_sma_3,@out_sma_4,@out_sma_5,@out_sma_6);
			
			INSERT INTO `ying_calc`.`index_xts_sma` (`dt`,`idi`,`cjezb5`,`cjezb10`,`cjezb20`,`cjezb30`,`cjezb60`,`cjezb120`) VALUES (cursor_fetch_tmp_dt, cursor_fetch_tmp_idi, @out_sma_1, @out_sma_2, @out_sma_3, @out_sma_4, @out_sma_5, @out_sma_6) ON DUPLICATE KEY UPDATE `cjezb5` =  @out_sma_1, `cjezb10` =  @out_sma_2, `cjezb20` =  @out_sma_3, `cjezb30` =  @out_sma_4, `cjezb60` =  @out_sma_5, `cjezb120` =  @out_sma_6;
...
DELIMITER ;
	...
	CALL `ying_calc`.`index_xts_cjezb_sma_multiPeriods_loop_dt_idi`('2015-08-12 15:05:00' , '2016-08-08 15:05:00', 5, 10, 20, 30, 60, 120);     

	-- CALL `ying_calc`.`index_xts_cjezb_sma_multiPeriods_loop_dt_idi`(DATE_SUB(CURRENT_TIMESTAMP(), INTERVAL 3 DAY) , '2019-10-15 15:05:00', 5, 10, 20, 30, 60, 120);     
