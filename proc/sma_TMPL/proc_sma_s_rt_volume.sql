--  test query in the proc    
	CALL `sma_s_rt_volume_loop_dt_ids`('2015-09-20 13:05:00', '2019-10-15 15:05:00', 5, 10, 20, 30, 60, 120);     
    
	SELECT * FROM `ying`.`s_rt_sma` WHERE dt >= '2005-10-13 13:05:00' AND dt <= '2019-10-15 15:05:00' ORDER BY `dt` DESC;
 
DROP PROCEDURE IF EXISTS `sma_s_rt_volume_loop_dt_ids`;

DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `sma_s_rt_volume_loop_dt_ids`(
