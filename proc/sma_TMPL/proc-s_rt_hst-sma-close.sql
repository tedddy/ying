...
DROP PROCEDURE IF EXISTS `ying_calc`.`s_rt_hst_sma_close`;
-- This procedure compute sma (simple moving average) for stock (ids) at given datetime (`dt`).
DELIMITER $$ 
CREATE PROCEDURE `ying_calc`.`s_rt_hst_sma_close`(
	IN 	
...
-- 	DECLARE coursors 
		DECLARE cursor1 CURSOR FOR -- variable for the first cursor. 
			SELECT `close` FROM `ying_calc`.`s_rt_hst` WHERE `ids` = in_ids and `dt` <= in_dt ORDER BY `dt` DESC limit limit_number_for_cursor;
...
CALL `ying_calc`.`s_rt_hst_sma_close_loop`(date_sub(current_timestamp(), INTERVAL 3 MINUTE ), '2018-08-08 15:05:00');

-- 	CALL `ying_calc`.`s_rt_hst_sma_close_loop`('2015-11-10 13:35:00', '2018-08-08 15:05:00');     
--     
-- 	SELECT * FROM `ying_calc`.`s_rt_hst_sma` WHERE dt >= '2005-10-13 13:05:00' AND dt <= '2019-10-15 15:05:00' ORDER BY `dt` DESC;
--  
DROP PROCEDURE IF EXISTS `ying_calc`.`s_rt_hst_sma_close_loop`;
-- 
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `ying_calc`.`s_rt_hst_sma_close_loop`(
...
-- 	open cursor
	...
	-- 	Loop of cursor  
		...
		-- 	actions	
			--  sma_1
				CALL `s_rt_hst_sma_close`(cursor_fetch_tmp_dt, cursor_fetch_tmp_ids, in_smaPeriods_1, @out_sma_1);
			--  sma_2
				CALL `s_rt_hst_sma_close`(cursor_fetch_tmp_dt, cursor_fetch_tmp_ids, in_smaPeriods_2, @out_sma_2);	
			--  sma_3
				CALL `s_rt_hst_sma_close`(cursor_fetch_tmp_dt, cursor_fetch_tmp_ids, in_smaPeriods_3, @out_sma_3);
			--  sma_4
				CALL `s_rt_hst_sma_close`(cursor_fetch_tmp_dt, cursor_fetch_tmp_ids, in_smaPeriods_4, @out_sma_4);
			--  sma_5
				CALL `s_rt_hst_sma_close`(cursor_fetch_tmp_dt, cursor_fetch_tmp_ids, in_smaPeriods_5, @out_sma_5);	
			--  sma_6
				CALL `s_rt_hst_sma_close`(cursor_fetch_tmp_dt, cursor_fetch_tmp_ids, in_smaPeriods_6, @out_sma_6);	
			IF cursor_fetch_tmp_dt IS NOT NULL THEN
				INSERT INTO `ying_calc`.`s_rt_hst_sma` (`dt`,`ids`,`close5`,`close10`,`close20`,`close30`,`close60`,`close120`) VALUES (cursor_fetch_tmp_dt, cursor_fetch_tmp_ids, @out_sma_1, @out_sma_2, @out_sma_3, @out_sma_4, @out_sma_5, @out_sma_6) ON DUPLICATE KEY UPDATE `close5` =  @out_sma_1, `close10` =  @out_sma_2, `close20` =  @out_sma_3, `close30` =  @out_sma_4, `close60` =  @out_sma_5, `close120` =  @out_sma_6;
                        END IF;
...
--     
-- 	CALL `s_rt_hst_sma_close_multiPeriods`('2019-09-09 00:00:00', '601318', 5, 10, 20, 30, 60, 120, @out_sma_1, @out_sma_2, @out_sma_3, @out_sma_4, @out_sma_5, @out_sma_6);
-- 	SELECT @out_sma_1, @out_sma_2, @out_sma_3, @out_sma_4, @out_sma_5, @out_sma_6;
-- 
-- DROP PROCEDURE IF EXISTS `s_rt_hst_sma_close_multiPeriods`;
-- 


-- instructions for PROCEDURE `s_rt_hst_sma_close`
...
-- Test the proc
-- 	CALL `s_rt_hst_sma_close`('2019-09-09', '601318', 10, @out_sma);
-- 	SELECT @out_sma;    

-- DROP PROCEDURE IF EXISTS `s_rt_hst_sma_close_multiPeriods`;

-- DELIMITER $$
-- CREATE DEFINER=`gxh`@`%` PROCEDURE `s_rt_hst_sma_close_multiPeriods`
	-- (
	-- IN 	
		-- in_dt DATETIME, -- variable datetime. If the value of this variable is large (such as '2019-09-09'), then the sma we get from this proc is for the latest datetime in table s_rt_hst.  
		-- ids VARCHAR(6),
...
-- --  sma_1
	-- CALL `s_rt_hst_sma_close`(in_dt, ids, in_smaPeriods_1, @out_sma_1);		
	-- SET out_sma_1 = @out_sma_1;
    
-- --  sma_2
	-- CALL `s_rt_hst_sma_close`(in_dt, ids, in_smaPeriods_2, @out_sma_2);		
	-- SET out_sma_2 = @out_sma_2;

-- --  sma_3
	-- CALL `s_rt_hst_sma_close`(in_dt, ids, in_smaPeriods_3, @out_sma_3);		
	-- SET out_sma_3 = @out_sma_3;

-- --  sma_4
	-- CALL `s_rt_hst_sma_close`(in_dt, ids, in_smaPeriods_4, @out_sma_4);		
	-- SET out_sma_4 = @out_sma_4;

-- --  sma_5
	-- CALL `s_rt_hst_sma_close`(in_dt, ids, in_smaPeriods_5, @out_sma_5);		
	-- SET out_sma_5 = @out_sma_5;

-- --  sma_6
	-- CALL `s_rt_hst_sma_close`(in_dt, ids, in_smaPeriods_6, @out_sma_6);		
