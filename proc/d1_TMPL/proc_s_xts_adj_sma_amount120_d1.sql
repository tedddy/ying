...
-- 
-- 	going to replace `tableTMPL` with `s_xts_adj_sma`, `idTMPL` with `ids`, `dtTMPL` with `dt`, `fieldTMPL` with `amount120`
-- 	save file proc_tableTMPL_idTMPL_dtTMPL_fieldTMPL_d1.sql as proc_s_xts_adj_sma_amount120_d1.sql
-- 	replace
...
-- 	execute file proc_s_xts_adj_sma_amount120.sql & test all procedures
...
DROP PROCEDURE IF EXISTS `ying_calc`.`s_xts_adj_sma_amount120_d1`;

DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `ying_calc`.`s_xts_adj_sma_amount120_d1`()
BEGIN
...
-- Reference: file E:\bYun\securities\ying\learning\learning.sql; search key words: learning and researching E:\bYun\securities\ying\proc\d1_TMPL\proc_s_xts_adj_sma_amount120_d1.sql 
