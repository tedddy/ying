...
DROP PROCEDURE IF EXISTS `ying_calc`.`s_xts_adj_sma_ids_d_close120_d1_cur_day`;

DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `ying_calc`.`s_xts_adj_sma_ids_d_close120_d1_cur_day`()
BEGIN
...
CALL `ying_calc`.`s_xts_adj_sma_ids_d_close120_d1_cur_day`;

SELECT `ids`, `d`, `close120`, `close120_d1` FROM `ying_calc`.`s_xts_adj_sma` ORDER BY ids, d DESC;
