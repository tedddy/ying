...
SELECT dt, idi, rate5, z - d AS dif_zd, zW - dW AS dif_zdW, zfg - dfg AS dif_zdfg, zfgW - dfgW AS dif_zdfgW, zg - dg AS dif_zdg, zgW - dgW AS dif_zdgW, zt - dt AS dif_zdt, ztW - d10W AS dif_zd10W FROM `ying`.`index_cons_stat_zd` WHERE  DATE(dt) = CURDATE(); -- date(dt) = curdate() - 1 or 

-- SELECT zd.dt AS '时间', zd.z AS '上涨数', zd.d AS '下跌数', zdfg.zfg AS '涨3.82', zdfg.dfg AS '跌3.82' , zdg.zg AS '涨6.18', zdg.dg AS '跌6.18' FROM `ying`.`hs_s_index_stat_z_d_No` zd join `ying`.`hs_s_index_stat_zfg_dfg_No` zdfg on (zd.dt = zdfg.dt)  join `ying`.`hs_s_index_stat_zg_dg_No` zdg on (zd.dt = zdg.dt);
