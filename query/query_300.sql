...
SELECT * FROM `ying`.`index_cons_stat_zd` where idi = '000016' order by `dt` desc;

SELECT * FROM `ying`.`index_cons_stat_zd` where idi = '399102' order by `dt` desc;

SELECT * FROM `ying`.`index_cons_stat_zd` where idi = '399807' order by `dt` desc;

SELECT * FROM `ying`.`index_cons_stat_zd` where time(dt) = "15:05:00" order by `dt` desc;
