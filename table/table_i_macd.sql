...
SELECT * FROM ying_calc.i_macd order by dt desc, bl_cl;

-- DROP TABLE `ying_calc`.`i_macd`;
CREATE TABLE `ying_calc`.`i_macd` (
...
CALL `ying_calc`.`i_macd_import`;

-- DROP PROCEDURE `ying_calc`.`i_macd_import`;
DELIMITER $$
CREATE DEFINER=`gxh`@`%` PROCEDURE `ying_calc`.`i_macd_import`()
BEGIN
INSERT INTO `ying_calc`.`i_macd`
(`idi`,
`dt`,
`close`,
`macd_cl`,
